...
def run_one_experiment(
    ...
    for ep in range(epochs):
        ...
        for loss_type in ["recon", "impute"]:
            for mod, val in tr["modality_losses"][loss_type].items():
                k = f"{loss_type}_{mod}"
                if k not in train_hist:
                    train_hist[k] = []
                train_hist[k].append(val)
            for mod, val in vl["modality_losses"][loss_type].items():
                k = f"{loss_type}_{mod}"
                if k not in val_hist:
                    val_hist[k] = []
                val_hist[k].append(val)
